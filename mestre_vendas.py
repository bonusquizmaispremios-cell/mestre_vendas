...
defaults = {
    ...
    'mv_conquistas':      [],
    'mv_vendas_total':    0,
    'mv_receita_total':   0.0,
    'mv_funil':           {},
    'mv_testes_ab':       [],
    'mv_kit_lancamento':  None,
    'mv_nivel':           'Iniciante',
...
# TELA: LOGIN
# ============================================================
if 'mv_testes_ab' not in st.session_state: st.session_state['mv_testes_ab'] = []
...
if st.session_state.etapa == "Login":
    ...
    with _tab_Home:
        ...
        with col_r:
            if st.button("🚪 Sair", key="mestreve3"):
        ...
        if st.session_state.historico_analises:
            ...
            for item in reversed(st.session_state.historico_analises[-4:]):
                st.markdown(
                    ...
                    f"<span class='badge-azul'>{item.get('produto', '')[:30]}</span> "
    ...
    with _tab_Diagnostico:
        ...
        with col1:
            produto   = st.text_input("📦 Produto:", value=st.session_state.produto_padrao, placeholder="ex: comedouro para pássaros", key="mestreve4")
            descricao = st.text_area("📝 Descreva o produto:", height=70, placeholder="ex: comedouro artesanal em madeira, feito à mão, tamanho médio...", key="mestreve5")
            concorrentes = st.text_input("🏆 Concorrentes:", placeholder="ex: lojas do Mercado Livre, ateliês locais...", key="mestreve6")
            objetivo  = st.number_input("🎯 Meta mensal (R$):", min_value=0.0, value=float(st.session_state.mv_objetivo or 0), step=100.0, key="mestreve7")
            st.session_state.mv_objetivo = objetivo
        with col2:
            nicho     = st.text_input("🎯 Nicho:", value=st.session_state.nicho_padrao, placeholder="ex: pet, casa, beleza...", key="mestreve8")
            custo     = st.number_input("💸 Custo do produto (R$):", min_value=0.0, value=float(st.session_state.preco_custo), step=0.5, key="mestreve9")
            preco     = st.number_input("💰 Preço de venda (R$):", min_value=0.0, value=float(st.session_state.preco_venda), step=0.5, key="mestreve10")
            canal     = st.selectbox("📲 Canal atual:", ["WhatsApp","Instagram","Mercado Livre","Shopee","Loja própria","TikTok","Facebook","Físico/local","Ainda não vendo"], key="mestreve11")

        col3, col4, col5 = st.columns(3)
        with col3:
            capital   = st.number_input("💵 Capital disponível (R$):", min_value=0.0, value=float(st.session_state.mv_capital or 0), step=50.0, key="mestreve12")
            st.session_state.mv_capital = capital
        with col4:
            experiencia = st.selectbox("📚 Experiência em vendas:", ["Nenhuma","Pouca (menos de 1 ano)","Média (1-3 anos)","Avançada (3+ anos)"], key="mestreve13")
            st.session_state.mv_experiencia = experiencia
        with col5:
            tempo     = st.number_input("⏰ Horas por dia disponíveis:", min_value=1, max_value=24, value=int(st.session_state.mv_tempo or 2), key="mestreve14")
            st.session_state.mv_tempo = tempo

        col6, col7, col8 = st.columns(3)
        with col6:
            estoque   = st.radio("📦 Tem estoque/produto em mãos?", ["Sim","Não"], horizontal=True, key="mestreve15")
        with col7:
            internet  = st.selectbox("💻 Habilidade com internet:", ["Básico","Intermediário","Avançado"], key="mestreve16")
            st.session_state.mv_internet = internet
        with col8:
            risco     = st.selectbox("⚖️ Tolerância a risco:", ["Baixa — prefiro segurança","Média","Alta — topo tudo"], key="mestreve17")
            st.session_state.mv_risco = risco

        # Preview financeiro em tempo real
        if custo > 0 and preco > 0:
            lucro_unit = preco - custo
            margem = round(lucro_unit / preco * 100, 1)
            unidades_meta = round(objetivo / lucro_unit) if lucro_unit > 0 and objetivo > 0 else 0
            unidades_dia  = round(unidades_meta / 30, 1) if unidades_meta > 0 else 0
            cor_m = "#059669" if margem >= 40 else ("#B45309" if margem >= 20 else "#B91C1C")
            st.markdown(f"""
            <div style='background:#FFF8F0;border:2px solid {cor_m};border-radius:12px;padding:14px 18px;margin:10px 0;'>
            <div style='display:grid;grid-template-columns:repeat(4,1fr);gap:12px;text-align:center;'>
                <div><div style='font-size:0.72em;color:#92400E;'>💸 LUCRO/UNIDADE</div><div style='font-size:1.6em;font-weight:700;color:{cor_m};'>R${lucro_unit:.2f}</div></div>
                <div><div style='font-size:0.72em;color:#92400E;'>📊 MARGEM</div><div style='font-size:1.6em;font-weight:700;color:{cor_m};'>{margem}%</div></div>
                <div><div style='font-size:0.72em;color:#92400E;'>🎯 UNID/MÊS PARA META</div><div style='font-size:1.6em;font-weight:700;color:#1D4ED8;'>{unidades_meta}</div></div>
                <div><div style='font-size:0.72em;color:#92400E;'>📅 UNID/DIA</div><div style='font-size:1.6em;font-weight:700;color:#1D4ED8;'>{unidades_dia}</div></div>
            </div>
            </div>
            """, unsafe_allow_html=True)

        if st.button("🧠 GERAR DIAGNÓSTICO COMPLETO", use_container_width=True, key="mestreve18"):
            if produto.strip():
                with st.spinner("Analisando seu negócio..."):
                    margem_calc = round((preco-custo)/preco*100,1) if preco > 0 else 0
                    lucro_calc  = round(preco - custo, 2)
                    un_meta     = round(objetivo / lucro_calc) if lucro_calc > 0 and objetivo > 0 else "?"
                    un_dia      = round(objetivo / lucro_calc / 30, 1) if lucro_calc > 0 and objetivo > 0 else "?"

                    prompt = (
                        f"Faça um diagnóstico completo e personalizado de negócio de produto físico.\n"
                        f"Todos os dados são REAIS — use-os nos cálculos:\n\n"
                        f"DADOS DO NEGÓCIO:\n"
                        f"- Produto: {produto}\n- Descrição: {descricao or 'não informada'}\n"
                        f"- Nicho: {nicho}\n- Concorrentes: {concorrentes or 'não informado'}\n"
                        f"- Custo: R${custo:.2f}\n- Preço de venda: R${preco:.2f}\n"
                        f"- Lucro por venda: R${lucro_calc:.2f}\n- Margem: {margem_calc}%\n"
                        f"- Canal atual: {canal}\n"
                        f"- Meta mensal: R${objetivo:.0f}\n"
                        f"- Capital disponível: R${capital:.0f}\n"
                        f"- Experiência: {experiencia}\n- Horas/dia: {tempo}h\n"
                        f"- Tem estoque: {estoque}\n- Internet: {internet}\n- Risco: {risco}\n\n"
                        f"FORMATO OBRIGATÓRIO:\n\n"
                        f"📋 SEU DIAGNÓSTICO\n\n"
                        f"| Campo | Dado informado |\n|---|---|\n"
                        f"| Produto | {produto} |\n"
                        f"| Custo | R${custo:.2f} |\n"
                        f"| Preço de venda | R${preco:.2f} |\n"
                        f"| Lucro líquido/venda | R${lucro_calc:.2f} |\n"
                        f"| Margem | {margem_calc}% |\n"
                        f"| Meta mensal | R${objetivo:.0f} |\n"
                        f"| Unidades/mês para a meta | {un_meta} |\n"
                        f"| Unidades/dia | {un_dia} |\n"
                        f"| Capital disponível | R${capital:.0f} |\n"
                        f"| Experiência | {experiencia} |\n\n"
                        f"---\n\n"
                        f"🏆 MODELO MAIS INDICADO: [nome do modelo]\n"
                        f"Compatibilidade com seu perfil: [X]/100\n\n"
                        f"📊 COMPARAÇÃO DE MODELOS:\n\n"
                        f"| Modelo | Investimento | Risco | Margem | Velocidade | Pontuação |\n"
                        f"|---|---|---|---|---|---|\n"
                        f"[liste os 6 modelos principais com pontuação real baseada no perfil]\n\n"
                        f"---\n\n"
                        f"💰 ANÁLISE FINANCEIRA REAL:\n\n"
                        f"Seu objetivo é R${objetivo:.0f}/mês.\n"
                        f"Com lucro líquido de R${lucro_calc:.2f}/venda, você precisaria de {un_meta} vendas/mês ({un_dia}/dia).\n\n"
                        f"📈 CENÁRIOS (baseados nos seus números reais):\n\n"
                        f"| Cenário | Vendas/mês | Receita |\n|---|---|---|\n"
                        f"| 🟡 Conservador (20% da meta) | [X] | R$[X] |\n"
                        f"| 🟠 Provável (60% da meta) | [X] | R$[X] |\n"
                        f"| 🟢 Otimista (meta atingida) | {un_meta} | R${objetivo:.0f} |\n\n"
                        f"---\n\n"
                        f"📢 CANAIS RECOMENDADOS (em ordem de prioridade):\n"
                        f"[Canal 1]: [por quê é o melhor para esse perfil]\n"
                        f"[Canal 2]: [quando ativar]\n"
                        f"[Canal 3]: [a longo prazo]\n\n"
                        f"---\n\n"
                        f"🗓️ PLANO DE AÇÃO:\n\n"
                        f"**HOJE:** [ação concreta — tempo estimado]\n"
                        f"**AMANHÃ:** [ação concreta]\n"
                        f"**SEMANA 1 (dias 3-7):** [ações]\n"
                        f"**SEMANA 2:** [ações]\n"
                        f"**MÊS 2:** [ações de escala]\n\n"
                        f"---\n\n"
                        f"🎯 PRÓXIMA AÇÃO:\n"
                        f"Faça [ação específica].\n"
                        f"Por quê? [razão objetiva]\n"
                        f"Tempo estimado: [X minutos/horas]\n"
                        f"Resultado esperado: [o que vai acontecer]"
                    )
                    res = vendas_ia(prompt)
                    salvar_analise("Diagnóstico", produto, res)
                    st.session_state.produto_padrao = produto
                    st.session_state.nicho_padrao   = nicho
                    st.session_state.preco_custo    = custo
                    st.session_state.preco_venda    = preco
                    st.session_state['diag_temp'] = res
            else:
                st.warning("Informe o produto.")

        if st.session_state.get('diag_temp'):
            st.markdown(st.session_state['diag_temp'])
            ...
            with col_sv:
                if st.button("💾 Salvar", use_container_width=True, key="mestreve19"):
                    st.session_state.materiais_salvos.append({
                        'tipo':'Diagnóstico','produto':st.session_state.produto_padrao,
    ...
    with _tab_ModeloVenda:
        ...
        if st.button("🎯 DEFINIR MEU MODELO IDEAL", key="mestreve20"):
            if produto_m.strip():
        ...
        if st.session_state.get('modelo_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar (.txt)", data=st.session_state['modelo_temp'], file_name="modelo_venda.txt", mime="text/plain", use_container_width=True, key="mestreve21")
            with col_sv:
    ...
    with _tab_Oferta:
        ...
        if st.button("🛒 CRIAR OFERTA IRRESISTÍVEL", key="mestreve22"):
            if produto_o.strip():
        ...
        if st.session_state.get('oferta_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar oferta (.txt)", data=st.session_state['oferta_temp'], file_name="estrategia_oferta.txt", mime="text/plain", use_container_width=True, key="mestreve23")
            with col_sv:
    ...
    with _tab_Precificacao:
        ...
        if preco_atual > 0 and custo_prod > 0:
            ...
            cor = "#059669" if margem_real >= 40 else ("#B45309" if margem_real >= 20 else "#B91C1C")
        ...
        if st.button("💰 GERAR ANÁLISE COMPLETA DE PRECIFICAÇÃO", key="mestreve24"):
        ...
        if st.session_state.get('prec_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar (.txt)", data=st.session_state['prec_temp'], file_name="precificacao.txt", mime="text/plain", use_container_width=True, key="mestreve25")
            with col_sv:
    ...
    with _tab_Avatar:
        ...
        if st.button("👤 CRIAR AVATAR COMPLETO", key="mestreve26"):
            if produto_av.strip():
        ...
        if st.session_state.get('avatar_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar avatar (.txt)", data=st.session_state['avatar_temp'], file_name="avatar_cliente.txt", mime="text/plain", use_container_width=True, key="mestreve27")
            with col_sv:
    ...
    with _tab_Copy:
        ...
        if st.button("✍️ GERAR COPIES AGORA", key="mestreve28"):
            if produto_c.strip():
        ...
        if st.session_state.get('copy_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar copies (.txt)", data=st.session_state['copy_temp'], file_name="copies_produto.txt", mime="text/plain", use_container_width=True, key="mestreve29")
            with col_sv:
    ...
    with _tab_Conteudo:
        ...
        if st.button("📱 GERAR CONTEÚDO", key="mestreve30"):
            if produto_ct.strip():
        ...
        if st.session_state.get('cont_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar conteúdo (.txt)", data=st.session_state['cont_temp'], file_name="conteudo_redes.txt", mime="text/plain", use_container_width=True, key="mestreve31")
            with col_sv:
    ...
    with _tab_Criativos:
        ...
        if st.button("🎥 GERAR CRIATIVOS", key="mestreve32"):
            if produto_cr.strip():
        ...
        if st.session_state.get('cri_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar criativos (.txt)", data=st.session_state['cri_temp'], file_name="criativos_anuncios.txt", mime="text/plain", use_container_width=True, key="mestreve33")
            with col_sv:
    ...
    with _tab_Fechamento:
        ...
        with tab1:
            ...
            if st.button("💬 GERAR SCRIPTS DE FECHAMENTO", key="mestreve34"):
                if produto_f.strip():
            ...
            if st.session_state.get('fech_temp'):
                col_dl, col_sv = st.columns(2)
                with col_dl:
                    st.download_button("📋 Baixar scripts (.txt)", data=st.session_state['fech_temp'], file_name="scripts_fechamento.txt", mime="text/plain", use_container_width=True, key="mestreve35")
                with col_sv:
        ...
        with tab2:
            ...
            if st.session_state.get('obj_temp'):
                col_dl, col_sv = st.columns(2)
                with col_dl:
                    st.download_button("📋 Baixar (.txt)", data=st.session_state['obj_temp'], file_name="quebra_objecoes.txt", mime="text/plain", use_container_width=True, key="mestreve36")
                with col_sv:
    ...
    with _tab_Atendimento:
        ...
        if st.button("🤖 GERAR ATENDIMENTO AUTOMÁTICO", key="mestreve37"):
            if produto_at.strip():
        ...
        if st.session_state.get('at_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar (.txt)", data=st.session_state['at_temp'], file_name="atendimento_automatico.txt", mime="text/plain", use_container_width=True, key="mestreve38")
            with col_sv:
    ...
    with _tab_Marketplace:
        ...
        if st.button("🛍️ GERAR ESTRATÉGIA DE MARKETPLACE", key="mestreve39"):
            if produto_mk.strip():
        ...
        if st.session_state.get('mk_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar estratégia (.txt)", data=st.session_state['mk_temp'], file_name=f"marketplace_{marketplace if 'marketplace' in dir() else ''}.txt", mime="text/plain", use_container_width=True, key="mestreve40")
            with col_sv:
    ...
    with _tab_Loja:
        ...
        if st.button("🌐 GERAR ESTRATÉGIA DE LOJA", key="mestreve41"):
            if produto_lv.strip():
        ...
        if st.session_state.get('lv_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar (.txt)", data=st.session_state['lv_temp'], file_name="loja_virtual.txt", mime="text/plain", use_container_width=True, key="mestreve42")
            with col_sv:
    ...
    with _tab_Trafego:
        ...
        if st.button("📢 GERAR ESTRATÉGIA DE TRÁFEGO", key="mestreve43"):
            if produto_tp.strip():
        ...
        if st.session_state.get('tp_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar estratégia (.txt)", data=st.session_state['tp_temp'], file_name="trafego_pago.txt", mime="text/plain", use_container_width=True, key="mestreve44")
            with col_sv:
    ...
    with _tab_Local:
        ...
        if st.button("📍 GERAR ESTRATÉGIA LOCAL", key="mestreve45"):
        ...
        if st.session_state.get('loc_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar (.txt)", data=st.session_state['loc_temp'], file_name="vendas_locais.txt", mime="text/plain", use_container_width=True, key="mestreve46")
            with col_sv:
    ...
    with _tab_Escala:
        ...
        if st.button("📈 GERAR PLANO DE ESCALA", key="mestreve47"):
            if produto_es.strip():
        ...
        if st.session_state.get('esc_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar plano (.txt)", data=st.session_state['esc_temp'], file_name="plano_escala.txt", mime="text/plain", use_container_width=True, key="mestreve48")
            with col_sv:
    ...
    with _tab_Validacao:
        ...
        if st.button("🧪 VALIDAR MEU PRODUTO", key="mestreve49"):
        ...
        if st.session_state.get('val_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar validação (.txt)", data=st.session_state['val_temp'], file_name="validacao_produto.txt", mime="text/plain", use_container_width=True, key="mestreve50")
            with col_sv:
    ...
    with _tab_Auditoria:
        ...
        if st.button("📊 AUDITAR MEU NEGÓCIO", key="mestreve51"):
            if produto_au.strip():
        ...
        if st.session_state.get('audit_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar auditoria (.txt)", data=st.session_state['audit_temp'], file_name="auditoria_negocios.txt", mime="text/plain", use_container_width=True, key="mestreve52")
            with col_sv:
    ...
    with _tab_Estoque:
        ...
        if st.button("📦 GERAR PLANO DE GESTÃO DE ESTOQUE", key="mestreve53"):
        ...
        if st.session_state.get('est_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar (.txt)", data=st.session_state['est_temp'], file_name="gestao_estoque.txt", mime="text/plain", use_container_width=True, key="mestreve54")
            with col_sv:
    ...
    with _tab_Branding:
        ...
        if st.button("💎 CRIAR IDENTIDADE DE MARCA", key="mestreve55"):
        ...
        if st.session_state.get('brand_temp'):
            col_dl, col_sv = st.columns(2)
            with col_dl:
                st.download_button("📋 Baixar identidade (.txt)", data=st.session_state['brand_temp'], file_name="identidade_marca.txt", mime="text/plain", use_container_width=True, key="mestreve56")
            with col_sv:
                    ...
                    if msg_sim.strip():
                        ...
                        with st.spinner(""):
                            try:
                                ...
                                if resp_txt: st.session_state['res_simuladorv_mestre1'] = resp_txt
                            except Exception as e:
                                resp_txt = f"⚠️ Erro: {e}"
    ...
    with _tab_Mentor:
        ...
        with col_env:
            if st.button("📤 CONSULTAR MENTOR", key="mestreve57"):
                if pergunta_m.strip():
        ...
        with col_limpar:
            if st.button("🗑️ Limpar", key="mestreve58"):
    ...
    with _tab_MeuNegocio:
        ...
        hist = st.session_state.historico_analises
        if not hist:
            st.info("Nenhuma análise ainda. Use os módulos para gerar conteúdo.")
        else:
            tipos = ["Todos"] + sorted(set(a['tipo'] for a in hist))
            filtro = st.selectbox("Filtrar por tipo:", tipos, key="mestreve79")
            filtrado = hist if filtro == "Todos" else [a for a in hist if a['tipo'] == filtro]
            st.markdown(f"**{len(filtrado)} item(s)**")
            for i, a in enumerate(reversed(filtrado)):
                with st.expander(f"[{a['tipo']}] {a['produto'][:50]} — {a['data']}"):
                    st.markdown(a['conteudo'][:1000] + ("..." if len(a['conteudo']) > 1000 else ""))
                    st.download_button("📋 Baixar", data=a['conteudo'],
    ...
    with _tab_Salvos:
        ...
        if not salvos:
            st.info("Nenhum material salvo ainda.")
        else:
            for i, s in enumerate(reversed(salvos)):
                with st.expander(f"[{s['tipo']}] {s.get('produto','')[:40]} — {s['data']}"):
                    st.markdown(s['conteudo'][:800] + ("..." if len(s['conteudo']) > 800 else ""))
                    col_dl, col_del = st.columns([3,1])
                    with col_dl:
                        st.download_button("📋 Baixar", data=s['conteudo'],
                            file_name="material_salvo.txt", mime="text/plain", key=f"dl_salvo_{i}")
                    with col_del:
